...
public class EnderHopperBlockEntity extends LootableContainerBlockEntity implements Hopper, Tickable {
  ...
  public EnderHopperBlockEntity() {
    super(EnderHopperMod.ENDER_HOPPER_ENTITY);
  ...
  public void fromTag(BlockState state, CompoundTag tag) {
    ...
    if (tag.contains("owner")) {
      this.owner = NbtHelper.toUuid(tag.get("owner"));
    }
  ...
  public CompoundTag toTag(CompoundTag tag) {
    ...
    if (owner != null) {
      tag.put("owner", NbtHelper.fromUuid(owner));
    }
  ...
  public static Inventory getInventoryAt(World world, BlockPos blockPos, UUID owner) {
    return getInventoryAt(world, (double) blockPos.getX() + 0.5D, (double) blockPos.getY() + 0.5D,
        (double) blockPos.getZ() + 0.5D, owner);
  }

  public static Inventory getInventoryAt(World world, double x, double y, double z, UUID owner) {
    ...
    if (block instanceof InventoryProvider) {
      ...
      PlayerEntity player = world.getServer().getPlayerManager().getPlayer(owner);
    ...
    if (inventory == null) {
      List<Entity> list = world.getEntities((Entity) null,
          new Box(x - 0.5D, y - 0.5D, z - 0.5D, x + 0.5D, y + 0.5D, z + 0.5D), EntityPredicates.VALID_INVENTORIES);
      if (!list.isEmpty()) {
  ...
  private Inventory getOutputInventory() {
    ...
    return getInventoryAt(this.getWorld(), this.pos.offset(direction), owner);
  }
  ...
  protected Text getContainerName() {
    return new TranslatableText("container.ender_hopper");
  }

  public static boolean extract(Hopper hopper, UUID owner) {
    Inventory inventory = getInputInventory(hopper, owner);
    if (inventory != null) {
  ...
  private static ItemStack transfer(Inventory from, Inventory to, ItemStack stack, int slot, Direction direction) {
    ...
    if (canInsert(to, stack, slot, direction)) {
      ...
      if (bl) {
        if (bl2 && to instanceof EnderHopperBlockEntity) {
          EnderHopperBlockEntity toEnderHopperBlockEntity = (EnderHopperBlockEntity) to;
          if (!toEnderHopperBlockEntity.isDisabled()) {
            int k = 0;
            if (from instanceof EnderHopperBlockEntity) {
              EnderHopperBlockEntity fromEnderHopperBlockEntity = (EnderHopperBlockEntity) from;
              if (toEnderHopperBlockEntity.lastTickTime >= fromEnderHopperBlockEntity.lastTickTime) {
                k = 1;
              }
            }

            toEnderHopperBlockEntity.setCooldown(8 - k);
          }
        }
  ...
  public static Inventory getInputInventory(Hopper hopper, UUID owner) {
  ...
  public void onEntityCollided(Entity entity) {
    if (entity instanceof ItemEntity) {
      ...
      if (VoxelShapes
          .matchesAnywhere(
              VoxelShapes.cuboid(entity.getBoundingBox().offset((double) (-blockPos.getX()),
                  (double) (-blockPos.getY()), (double) (-blockPos.getZ()))),
              this.getInputAreaShape(), BooleanBiFunction.AND)) {
        this.insertAndExtract(() -> {
